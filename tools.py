...
def bills_analytics(metric: Literal["by_status", "by_year", "top_committees"]) -> Dict[str, Any]:
    ...
    queries = {
        "by_status": "SELECT STATUS_DESC, COUNT(*) as count FROM POLICYPULSE_DB.PUBLIC.BILLS_ANALYTICS GROUP BY STATUS_DESC ORDER BY count DESC",
        "by_year": "SELECT YEAR, COUNT(*) as count FROM POLICYPULSE_DB.PUBLIC.BILLS_ANALYTICS GROUP BY YEAR ORDER BY YEAR DESC",
        "top_committees": "SELECT PRIMARY_COMMITTEE_NAME, COUNT(*) as count FROM POLICYPULSE_DB.PUBLIC.BILLS_ANALYTICS WHERE PRIMARY_COMMITTEE_NAME IS NOT NULL GROUP BY PRIMARY_COMMITTEE_NAME ORDER BY count DESC LIMIT 10"
    }
